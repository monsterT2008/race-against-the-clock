...
package net.mcreator.raceagainsttheclock.world.dimension;
...
import net.mcreator.raceagainsttheclock.procedures.TheroomPlayerEntersDimensionProcedure;
import net.mcreator.raceagainsttheclock.item.TheroomItem;
...
public class TheroomDimension extends RaceAgainstTheClockModElements.ModElement {
	@ObjectHolder("race_against_the_clock:theroom")
	public static final ModDimension dimension = null;
	@ObjectHolder("race_against_the_clock:theroom_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public TheroomDimension(RaceAgainstTheClockModElements instance) {
		super(instance, 1);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("theroom"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("race_against_the_clock:theroom")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("race_against_the_clock:theroom"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("race_against_the_clock:theroom"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("river")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new TheroomItem().setRegistryName("theroom"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("theroom_portal");
		}
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.addParticle(ParticleTypes.POOF, px, py, pz, vx, vy, vz);
			}
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(("block.ender_chest.close"))),
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = TheroomDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos)) || !(this.world.getBlockState(blockpos.down())
							.getBlock() == net.mcreator.minecraft.link.init.MCreatorLinkBlocks.LINK_BLOCK.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == net.mcreator.minecraft.link.init.MCreatorLinkBlocks.LINK_BLOCK
						.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == net.mcreator.minecraft.link.init.MCreatorLinkBlocks.LINK_BLOCK
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("theroom_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "theroom_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = TheroomDimension.CustomPortalBlock.createPatternHelper(this.world, blockpos);
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag
											? net.mcreator.minecraft.link.init.MCreatorLinkBlocks.LINK_BLOCK.getDefaultState().getBlock()
													.getDefaultState()
											: Blocks.AIR.getDefaultState());
						}
					}
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable,
								net.mcreator.minecraft.link.init.MCreatorLinkBlocks.LINK_BLOCK.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		if (event.getTo() == type) {
			{
				...
				TheroomPlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
